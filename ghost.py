...
class T_ghost():
    ...
    def plot_visibilities_pq(self, baseline, u=None, v=None, resolution=0, image_s=0, s=0):
        ...
        axs[1, 0].set_title("Real---$g_{%s,%s}(X_{%s,%s}^{-1}(u,v))$" % (
            str(baseline[0]), str(baseline[1]), str(baseline[0]), str(baseline[1])))
        ...
        axs[1, 1].set_title("Imaginary---$g_{%s,%s}(X_{%s,%s}^{-1}(u,v))$" % (
            str(baseline[0]), str(baseline[1]), str(baseline[0]), str(baseline[1])))
        ...
        axs[0, 0].set_title("Real---$r_{%s,%s}(X_{%s,%s}^{-1}(u,v))$" % (
            str(baseline[0]), str(baseline[1]), str(baseline[0]), str(baseline[1])))
        ...
        axs[0, 1].set_title("Imaginary---$r_{%s,%s}(X_{%s,%s}^{-1}(u,v))$" % (
            str(baseline[0]), str(baseline[1]), str(baseline[0]), str(baseline[1])))
        ...
        if u_temp <> None:
            u, v, V_G_pq, V_R_pq, phi, delta_b, theta, l_cor, m_cor = self.visibilities_pq_2D(baseline, u=u_temp,
                                                                                              v=v_temp,
                                                                                              resolution=resolution,
                                                                                              image_s=image_s, s=s)
            # baseline_new = [0,0]
    ...
    def determine_flux_pos_pq(self, baseline, i_max=1, number=10, resolution=250, image_s=3, s=2, sigma_t=0.05,
                              type_w_t="G-1", window=0.2, window_c=0.2):
        ...
        for i in xrange(ll.shape[0]):
            for j in xrange(ll.shape[1]):
                ...
                if not critical_region:
                    ...
                    image, l_v, m_v = self.sky_pq_2D(baseline, resolution, image_s, s, sigma=sigma_t, type_w=type_w_t,
                                                     avg_v=False, plot=False)
                    mask = self.create_mask(baseline)
    ...
    def determine_flux_A2(self, A_2_min=0.001, A_2_max=0.5, number=20, resolution=250, image_s=3, s=2, sigma_t=0.05,
                          type_w_t="GT-1", window=0.2):
        A_2_v = np.linspace(A_2_min, A_2_max, number)
        four = np.array(
            [(1, 0, 0), (1, self.l_0, -1 * self.m_0), (1, -1 * self.l_0, self.m_0), (1, 2 * self.l_0, -2 * self.m_0)])
        ...
        for i in xrange(len(A_2_v)):
            ...
            print "A_2 = ", A_2_v[i]
            self.A_2 = A_2_v[i]
            image, l_v, m_v = self.sky_2D(resolution, image_s, s, sigma=sigma_t, type_w=type_w_t, avg_v=False,
                                          plot=False)
    ...
    #
    #     c_eig = Calibration(s.ms, "all", "eig_cal",
    #                         s.total_flux)  # 0A sum of weight must be positive exception..., AB ---> value error
    ...
    def sky_2D(self, resolution, image_s, s, sigma=None, type_w="G-1", avg_v=False, plot=True, mask=False):
        ...
        if plot:
            ...
            fig = plt.figure()
            cs = plt.imshow(image.real, interpolation="bicubic", cmap="jet",
            ...
            fig = plt.figure()
            cs = plt.imshow(image.imag, interpolation="bicubic", cmap="jet",
                            extent=[l_cor[0], -1 * l_cor[0], m_cor[0], -1 * m_cor[0]])
            fig.colorbar(cs)
            self.plt_circle_grid(image_s)
